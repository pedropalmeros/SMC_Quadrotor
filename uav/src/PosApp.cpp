...
PosApp::PosApp(TargetController *controller): UavStateMachine(controller), behaviourMode(BehaviourMode_t::Default), vrpnLost(false) {
    ...
    xSMC = new xySMC(setup_myTab->At(0,0),"x Ctrl SMC");
    xSMC->UseDefaultPlot(graph_law_position->At(0,0));

    ySMC = new xySMC(setup_myTab->At(0,1),"y Ctrl SMC");
    ...
    **************************************************************************/
    ...
    getFrameworkManager()->AddDeviceToLog(xSMC);
    getFrameworkManager()->AddDeviceToLog(ySMC);
    ...
    /******************************************************
    /**** This is new ... Is to add in the VRPN TAB    ****/
    /******************************************************/
        MatrixDescriptor* desc_app=new MatrixDescriptor(3,1);
        desc_app->SetElementName(0,0,"xd");
        desc_app->SetElementName(1,0,"yd");
        desc_app->SetElementName(2,0,"yaw_d"); 

    DesiredPos_Matrix=new Matrix(this,desc_app,floatType,"Desired Pos");   
     GetUav()->GetImu()->AddDataToLog(DesiredPos_Matrix); 
     
     uavVrpn->xPlot()->AddCurve(DesiredPos_Matrix->Element(0,0),DataPlot::Blue,"xd");
     uavVrpn->yPlot()->AddCurve(DesiredPos_Matrix->Element(1,0),DataPlot::Blue,"yd");
     
     uavVrpn->XyPlot()->AddCurve(DesiredPos_Matrix->Element(1,0),DesiredPos_Matrix->Element(0,0),DataPlot::Blue,"xy_d");
     
     uav->GetAhrs()->YawPlot()->AddCurve(DesiredPos_Matrix->Element(2,0),DataPlot::Blue);
    //AddDataToLog(z_Ctrl);
...
/***************************************************************************************
    ExtraCheckPushButton  -> Checks whick button of the QtWidget has been clicked and changes the
                             ...
                             method
****************************************************************************************
// This Method, helps to verify the buttons of the GroundStation 
...
void PosApp::ExtraCheckJoystick(void) {
    if(GetTargetController()->IsButtonPressed(10) && (behaviourMode==BehaviourMode_t::Default)){  //R2
        OnlyAttitudeJoystick_mode();
    }
    if(GetTargetController()->IsButtonPressed(7) && (behaviourMode==BehaviourMode_t::Default)){  //L2
        ThrustMode();
    }
    if(GetTargetController()->IsButtonPressed(6) && (behaviourMode==BehaviourMode_t::Default)){  //L1
        xyControl();
        flag=false;
    }
...
void PosApp::ThrustMode(void){
    std::cout << "We might be in ThrustMode mode... Check the performance and good luck   " << endl;
    ...
    auto _OrientationMode = GetOrientationMode();
     ...
     if(_ThrustMode==ThrustMode_t::Custom){
        ...
        if(SetThrustMode(ThrustMode_t::Custom)){
            ...
            cout << "NO se ha podido cambiar a Thrust CUSTOM" << endl; 
        }
    }
...
void PosApp::ComputePositionControllers(flair::core::Vector3Df &Controller){
    ...
    if(flag){
        ...
        circulo();
        xSMC->SetValues(uav_pos.x,x_circ,uav_vel.x,dx_circ);
        xSMC->Update(GetTime());

        ySMC->SetValues(uav_pos.y,y_circ,uav_vel.y,dy_circ);
        ySMC->Update(GetTime());
        ...
        DesiredPos_Matrix->SetValue(0,0,x_circ);
       DesiredPos_Matrix->SetValue(1,0,y_circ);
...
void PosApp::ComputeDesiredQuaternion(flair::core::Quaternion &DesiredQuaternion){
    ...
    if(flag){
        ...
        DesiredPos_Matrix->SetValue(2,0,(yaw_rad*180)/3.1416);
