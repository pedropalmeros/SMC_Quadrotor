...
attQuatPD::attQuatPD(const LayoutPosition *position, string name)
    : ControlLaw(position->getLayout(), name,3) {
  pimpl_ = new attQuatPD_impl(this, position, name);
  
  SetIsReady(true);
}

attQuatPD::~attQuatPD(void) { delete pimpl_; }

void attQuatPD::UseDefaultPlot(const gui::LayoutPosition *position) {
  pimpl_->UseDefaultPlot(position);
}

void attQuatPD::Reset(void) {
  pimpl_->i = 0;
  pimpl_->first_update = true;
}

float attQuatPD::GetIntegral(void) const { return pimpl_->i; }

void attQuatPD::UpdateFrom(const io_data *data) {
  pimpl_->UpdateFrom(data);
  ProcessUpdate(output);
}


void attQuatPD::SetValues(float q0,        float q1,        float q2,        float q3,
