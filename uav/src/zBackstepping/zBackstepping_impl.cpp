...
zBackstepping_impl::zBackstepping_impl(zBackstepping *self, const LayoutPosition *position,
  ...
  kd = new DoubleSpinBox(reglages_groupbox->NewRow(), "kd:", -90000000, 90000000, 0.01);
  ki=new DoubleSpinBox(reglages_groupbox->NewRow(),"ki_z:",-500,1000,0.01,4);
...
void zBackstepping_impl::UpdateFrom(const io_data *data) {
  ...
  //float u = -kp->Value()*(e2-kp->Value()*e1) - kd->Value()*e2 - e1;
      ...
      // Hacemos la saturacioón
    float error_sat1 = (kp->Value()*e1) ; // kp(x-xd)
    float error_sat2 = (kd->Value()*e2);
    i += (ki->Value()*e1* delta_t);


    //saturación
//     if(error_sat1>satp->Value()) error_sat1=satp->Value();
//     if(error_sat1<-satp->Value()) error_sat1= -satp->Value();
// 
//     if(error_sat2> satd->Value()) error_sat2= satd->Value();
//     if(error_sat2<-satd->Value()) error_sat2= -satd->Value();
//     
//     if(i> sati->Value()) i= sati->Value();
//     if(i<-sati->Value()) i= -sati->Value();

    u=  -error_sat1 - i - error_sat2 ; // considerar a la ganancia kd como positiva

  
  //float u = -kp->Value()*e1 - kd->Value()*e2;
