...
attQuatBkstp_impl::attQuatBkstp_impl(attQuatBkstp *self, const LayoutPosition *position, string name) {
  ...
  k2=new DoubleSpinBox(reglages_groupbox->NewRow(),"kd:",-1000,1000,100);
  l2=new DoubleSpinBox(reglages_groupbox->NewRow(),"kp:",-1000,1000,100);
      
  k3=new DoubleSpinBox(reglages_groupbox->NewRow(),"kd_yaw:",-1000,1000,100);
  l3=new DoubleSpinBox(reglages_groupbox->NewRow(),"kp_yaw:",-1000,1000,100);
...
//             0, 0, Jz;
  ...
  //Tau  = S*J*Omega_bar-K*z_2-P.transpose()*z_1;
   // Tau =  -L*q_e - K*Omega;
  Tau.x = l2->Value()*q_e.x() + k2->Value()*Omega.x();
  Tau.y = l2->Value()*q_e.y() + k2->Value()*Omega.y();
  Tau.z = l3->Value()*q_e.z() + k3->Value()*Omega.z();
...
//   Ctrl_sat(2,0) = Saturation(Ctrl_sat(2,0),satQ->Value());
  ...
  self->output->SetValue(0, 0, Tau.x);
  self->output->SetValue(1, 0,Tau.y);
  self->output->SetValue(2, 0, Tau.z);
